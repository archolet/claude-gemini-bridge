"""
Centralized Tailwind CSS color definitions.

Single source of truth for all Tailwind color mappings used across the Gemini MCP server.

This module consolidates color definitions that were previously duplicated in:
- schemas.py (TAILWIND_COLOR_MAP)
- validation/contrast_checker.py (TAILWIND_COLORS)
- section_utils.py (_COLOR_PREFIXES, _TEXT_COLOR_PREFIXES)
...
# =============================================================================
# Reverse Mapping: HEX -> Tailwind Class
# =============================================================================

HEX_TO_TAILWIND_MAP: Dict[str, str] = {
    v.upper(): k for k, v in TAILWIND_COLOR_MAP.items() if v.startswith("#")
}


# =============================================================================
# Color Prefixes for O(1) Class Matching
# =============================================================================

# General color utility prefixes (for bg, border, etc.)
COLOR_PREFIXES: Tuple[str, ...] = (
    'bg-', 'border-', 'ring-', 'outline-',
    'divide-', 'from-', 'via-', 'to-'
)

# Text color prefixes with specific color families
TEXT_COLOR_PREFIXES: Tuple[str, ...] = (
...
# Typography class prefixes
TYPOGRAPHY_PREFIXES: Tuple[str, ...] = (
    'font-', 'tracking-', 'leading-', 'text-xs', 'text-sm', 'text-base',
    'text-lg', 'text-xl', 'text-2xl', 'text-3xl', 'text-4xl', 'text-5xl',
    'text-6xl', 'text-7xl', 'text-8xl', 'text-9xl'
)

# Spacing class prefixes
SPACING_PREFIXES: Tuple[str, ...] = (
    'p-', 'px-', 'py-', 'pt-', 'pb-', 'pl-', 'pr-', 'ps-', 'pe-',
    'm-', 'mx-', 'my-', 'mt-', 'mb-', 'ml-', 'mr-', 'ms-', 'me-',
    'gap-', 'gap-x-', 'gap-y-', 'space-x-', 'space-y-'
)

# Effects class prefixes
EFFECTS_PREFIXES: Tuple[str, ...] = (
    'shadow-', 'rounded-', 'blur-', 'opacity-', 'transition-',
...
# Color shade levels in Tailwind
COLOR_SHADES: Tuple[str, ...] = (
